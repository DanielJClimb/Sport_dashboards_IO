...
def duel_pitch(ax, player):
    ...
    ax_title = ax.set_title('Duels', fontsize=10)
    if len(duels) > 0:
    ...
    ax.legend(loc='lower center', ncol=3, fontsize=8, bbox_to_anchor=(0.5, -0.07))
    return
...
def pass_pitch(ax, player):
    ...
    pitch = Pitch(pitch_type='statsbomb', pitch_color='white', line_zorder=2, line_color='black', linewidth=1)
    pitch.draw(ax=ax, figsize=(16, 11))
    ax_title = ax.set_title('Passes', fontsize=10)
    if len(passes) > 0:
...
def recovery_pitch(ax, player):
    ...
    ax_title = ax.set_title('Ball recoveries', fontsize=10)
    if len(recoveries) > 0:
    ...
    ax.legend(loc='lower center', ncol=2, fontsize=8, bbox_to_anchor=(0.5, -0.07))
    return
...
def shot_pitch(ax, player):
    ...
    pitch = Pitch(pitch_type='statsbomb', pitch_color='white', line_zorder=2, line_color='black', linewidth=1)
    pitch.draw(ax=ax, figsize=(16, 16))
    ax_title = ax.set_title('Shots', fontsize=10)
    if len(shots) > 0:
...
def creating_stat_table(player, position):
    ...
    df1 = pd.DataFrame(list(zip(stats1)), columns=['Value'], index=names1)
    df2 = pd.DataFrame(list(zip(stats2)), columns=['Value'], index=names2)
    return df1, df2
...
st.subheader('Game performance', divider=True)
...
logofile = 'hudlstatsbomblogo.jpg'
st.image(logofile)
